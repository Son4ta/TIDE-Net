...
class Evaluator(object):
    ...
    def evaluate(self, true_batch, pred_batch):
        # [batch_size, seq_len, channel, h, w], 数据范围 [0., 1.]
    ...
    def done(self, is_main_process=True):
        ...
        for threshold in self.thresholds:
            ...
            # if threshold == 30:
            print_log('='*20 + f"Threshold: {threshold} with melthod 1"+'='*20, is_main_process)
        ...
        # np.save('/home/ices/CRA/linkh/alpha/res_save/ssim_res/ssim.npy', np.array(self.losses['ssim']))
        ...
        # 将 MSE 和 MAE 的平均值也添加到返回字典
        res_dict['mse'] = np.mean(mses)
        res_dict['mae'] = np.mean(mass)
